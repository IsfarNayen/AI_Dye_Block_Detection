# node_Analysis_details_main.py
import sys, os, shutil
from PyQt5 import QtWidgets, QtCore
from PyQt5.QtCore import Qt
from PyQt5.QtGui import QPixmap
from PIL import Image, ImageDraw, ImageFont
from ..UI.skeleton_UI_files.node_Analysis_details import Ui_MainWindow
...
class MainApp(QtWidgets.QMainWindow):
    ...
    def set_result_data(self, original_image_path, predicted_image_path, node_name=None, confidence=None):
        self.predicted_image_path = predicted_image_path
        
        self.set_image_in_frame(self.ui.originalImageframe, original_image_path)
        self.set_image_in_frame(self.ui.segmentedImageframe, predicted_image_path)

        if hasattr(self.ui, "resultMainLabel"):
            self.ui.resultMainLabel.setText(node_name if node_name else "No Detection")

        if hasattr(self.ui, "possibleClassLabel"):
            if confidence is not None:
                self.ui.possibleClassLabel.setText(f"Confidence: {confidence:.2%}")
            else:
                self.ui.possibleClassLabel.setText("Confidence: N/A")

        if hasattr(self.ui, "analysisLabel"):
            self.ui.analysisLabel.setText("Classified Node Type")

        if hasattr(self.ui, "resultSubLabel"):
            self.ui.resultSubLabel.setText("Technology Class")
    ...
    def populate_details(self, details_df, area_summary=None):
        ...
        for _, row in details_df.iterrows():
            ...
            hex_color = str(row["class_color_hex"])
